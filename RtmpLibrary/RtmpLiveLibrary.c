...
#include "COpenRtmpLocalUser.h"
#import  "LiveUIViewController.h"
#import <AVFoundation/AVFoundation.h>
#import "RtmpTouchMoveView.h"

COpenRtmpLocalUser*         m_pCOpenRtmpLocalUser = NULL;
...
//    m_rtmp_live_baseurl = [[NSString alloc]initWithFormat:@"%s",baseurl];
...
bool StartLiveWithType(const char *liveurl,int type,const char *funname,int restype,UIView *localview)
{
  ...
  if (m_isstartlive) {
    ...
    m_pCOpenRtmpLocalUser = new COpenRtmpLocalUser;
    __block bool ret=NO;
    dispatch_sync(m_rtmp_queue, ^{
...
//    
//    m_liveUIViewController = [[LiveUIViewController alloc]initWithNibName:@"LiveUIViewController" bundle: bundle ];
//    
...
void RtmpStartWatchWithView(const char * liveUrl,UIView* uiview)
{
    ...
    dispatch_sync(m_rtmp_queue, ^{
        ...
        ret =  m_pOpenRtmpRemoteUser->ConnectMediaServer ((__bridge void*)uiview,liveUrl,6);
